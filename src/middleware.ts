...
// Refer to iron-session for session rate limiting: https://github.com/vvo/iron-session
...
const rateLimit = new Ratelimit({
  redis: redis,
  limiter: Ratelimit.slidingWindow(10, "60 s"), // user cant send more than 10 request every min
  analytics: true,
});
...
export async function middleware(request: NextRequest) {
  try {
    ...
    // rate limiting off users ip address
    ...
    // add rate limit to response headers
